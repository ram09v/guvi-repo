      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden">
        ...
        <div className="p-4 overflow-y-auto flex-1 bg-gray-50">
          {cartItems.length === 0 ? (
            <div className="text-center py-12 flex flex-col items-center">
                ...
                <button onClick={onClose} className="mt-4 text-primary font-bold hover:underline">Start Shopping</button>
            </div>
          ) : (
              ...
              {cartItems.map((item) => {
                 ...
                 return (
                  <div key={item.id} className="flex items-center gap-4 bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                    <div className="w-16 h-16 shrink-0 bg-white p-1 rounded border border-gray-100">
                        <img src={item.image} alt={item.title} className="w-full h-full object-contain" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-gray-800 line-clamp-1">{item.title}</h4>
                      <p className="text-primary font-bold">₹{priceInINR.toLocaleString('en-IN')}</p>
                    </div>
                    <button
        ...
        <div className="p-4 border-t border-gray-100 bg-white">
            <div className="flex justify-end gap-3">
                ...
                <button
                    className="bg-primary text-white px-6 py-2 rounded-lg hover:bg-hover transition font-medium cursor-pointer shadow-md whitespace-nowrap"
                >
                    Checkout
                </button>
            </div>
