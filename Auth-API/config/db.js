...
module.exports = function connectDB() {
  const { NODE_ENV, DB_USERNAME, DB_PASSWORD, DB_HOST, DB_NAME, DB_PARAMS } =
    process.env;
  ...
  const localURI = "mongodb://localhost:27017/auth_app";

  const dbURI = NODE_ENV === "production" ? atlasURI : localURI;

  if (!dbURI) {
    throw new Error(
      "No MongoDB URI configured. Set MONGO_URI or DB_USERNAME/DB_PASSWORD/DB_HOST/DB_NAME.",
    );
  }
